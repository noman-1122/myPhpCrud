<?php
    ...
    if(isset($_POST['login']))
    {
        $email = $_POST["email"];
        $pass = $_POST["password"];
        $loginQuery = "SELECT * FROM stds WHERE stdEmail = '$email' AND stdPass = '$pass'";
        $islogin = mysqli_query($con,$loginQuery);
        if($row = mysqli_fetch_array($islogin))
        {
            $_SESSION['sessionEmail'] = $email;
            $_SESSION['sessionImg'] = $row["imgPath"];
            $_SESSION['sessionUserName'] = $row["stdName"];
            if(isset($_POST["remember"]))
            {
                setcookie("CookieEmail",$row["stdEmail"],time()+120);
                setcookie("CookiePass",$row["stdPass"],time()+120);
            }
            else{
